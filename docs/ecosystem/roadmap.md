...
- Browser Terminal MVP: scoped `terminal:open` dashboard page, server-side
  in-memory sessions, open/close audit events, and opt-in agent-side PTY runner
  enabled with `LATTICE_AGENT_ALLOW_TERMINAL=1`.
...
7. Terminal follow-up: transcript retention policy, idle timeouts, reconnect
   semantics, and clearer close/failure handling in the dashboard.
...
- Terminal already ships with scoped authorization, audit events, opt-in agent
  enablement, and open/close records; follow-up transcript/idle-timeout work
  must preserve that bar. Group-leader features must ship with server-side authorization,
